...
public class MessageDigestTest {
	...
	static void test1() throws NoSuchAlgorithmException, IOException {
		MessageDigest md = MessageDigest.getInstance("SHA");
		String str = "张三李四王五";
		byte[] b = md.digest(str.getBytes("iso-8859-1"));
		StringWriter sw = new StringWriter();
		for (byte c : b) {
			sw.write(Integer.toHexString(0xff & c).toUpperCase());
		}
		System.out.println(sw.toString());
		sw.close();
	}

	static void test2() throws NoSuchAlgorithmException, IOException {
		MessageDigest md5 = MessageDigest.getInstance("md5");
		String str = "张三李四王五";
		String result = new String(Base64.getEncoder().encode(md5.digest(str.getBytes("utf-8"))));
	...
	public static String getHash(String source, String hashType) {
		...
		try {
			...
			byte[] encryptStr = md.digest(); // 获得密文完成哈希计算,产生128 位（MD5）的长整数，即16个字节
	...
	static void test3() throws NoSuchAlgorithmException, IOException {
		String str = "张三李四王五";
	...
	public static void main(String[] args) throws NoSuchAlgorithmException, IOException {
		...
		System.out.println(getHash2("你好", "MD5"));
		System.out.println(getHash2("你好", "SHA"));
		test3();
	}
}
